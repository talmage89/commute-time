...
class NotificationService: NSObject, ObservableObject {
    ...
    func sendEventNotification(for event: Event, location: Location) {
        ...
        case .arrived:
            ...
            content.body = "Arrived at \(location.name ?? "Unknown Location") at \((event.timestamp ?? Date()).timeString())"
        case .left:
            content.title = "Departed"
            content.body = "Left \(location.name ?? "Unknown Location") at \((event.timestamp ?? Date()).timeString())"
        case .none:
            return
        }
    ...
    func scheduleInactivityReminderIfNeeded() {
        ...
        do {
            ...
            let daysThreshold = Int(settings?.daysWithoutEventForReminder ?? Int32(Constants.defaultInactivityReminderDays))
