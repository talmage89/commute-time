...
class TripPairingService: ObservableObject {
    ...
    func processEvent(_ event: Event) {
        logger.info("Processing event: \(event.eventKind?.rawValue ?? "unknown") for location \(event.locationId)")
        ...
        updateAppSettings(lastEventAt: event.timestamp)
    }
    ...
    private func isPendingTripExpired(pendingTrip: Trip) -> Bool {
        ...
        let expiryTime = startEvent.timestamp.addingTimeInterval(TimeInterval(Constants.pendingTripExpiryHours * 3600))
